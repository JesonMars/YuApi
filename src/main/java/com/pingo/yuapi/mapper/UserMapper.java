...
public interface UserMapper {
         ...
         * 创建新用户
         */
        @Insert("INSERT INTO users (id, name, phone, avatar, community, wechat_openid, wechat_unionid) " +
                        "VALUES (#{id}, #{name}, #{phone}, #{avatar}, #{community}, #{wechatOpenid}, #{wechatUnionid})")
        ...
        @Update("UPDATE users SET vehicle_brand = #{vehicleBrand}, vehicle_color = #{vehicleColor}, " +
                        "plate_number = #{plateNumber} WHERE id = #{id}")
         ...
         * 更新认证状态
         */
        @Update("UPDATE users SET verification_status = #{status} WHERE id = #{id}")
        int updateVerificationStatus(@Param("id") String id, @Param("status") String status);

        /**
         * 更新首次设置完成状态
         */
        @Update("UPDATE users SET first_setup_completed = #{completed} WHERE wechat_openid = #{openid}")
